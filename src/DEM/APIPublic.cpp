...
inline bool computeClumpUnionMassPropsApprox(const DEMClumpTemplate& clump,
    ...
    for (size_t i = 0; i < max_samples; i++) {
        ...
        if (m2_sum > 1e-14) {
            const double var_sum = std::max(0.0, stats[4].var()) + std::max(0.0, stats[5].var()) + std::max(0.0, stats[6].var());
...
void DEMSolver::ShowMemStats() const {
    ...
    DEME_PRINTF("DEME-managed device memory peak: %s\n", pretty_format_bytes(GetTrackedDevicePeakMemUsage()).c_str());
...
std::shared_ptr<DEMMesh> DEMSolver::AddMesh(DEMMesh& mesh) {
    ...
    if (!mesh.mass_specified || !mesh.moi_specified) {
        ...
        if (volume > 1e-20 && std::isfinite(volume) && length(unit_inertia) > 1e-20 && std::isfinite(length(unit_inertia))) {
